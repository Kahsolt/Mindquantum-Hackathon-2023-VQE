...
def get_mol() -> MolecularData:
  geometry = get_mol_geo(args.molecule, fmt='mq')
  basis = 'sto3g'
  spin = 0
...
def optim_mindquantum(mol_pqc:MQAnsatzOnlyLayer, lr=4e-2, eps=1e-8):
  ...
  print("Optimized amplitudes: \n", mol_pqc.weight.asnumpy())


def optim_go(args, grad_ops:Callable, circ:Circuit, func:Callable, ansatz_circuit:Circuit=None):
  if args.init == 'ccsd':   # use estimated data from run_pyscf()
    ...
    init_amp = np.zeros(len(circ.params_name)) + 1e-6

  if args.optim == 'sp':
    optim_scipy(func, init_amp, grad_ops)
  elif args.optim == 'mq':
    mol_pqc = MQAnsatzOnlyLayer(grad_ops, args.init if args.init != 'ccsd' else 'normal')
    if args.init == 'ccsd':
...
def run_mindquantum(args, mol:MolecularData):
  ...
  optim_go(args, grad_ops, circ, func, ansatz_circuit)
...
def run_qupack(args, mol:MolecularData):
  ...
  optim_go(args, grad_ops, circ, func)
...
if __name__ == '__main__':
  ...
  parser.add_argument('-I', '--init', default='zeros', choices=['zeros', 'normal', 'ccsd'], help='optimzer scipy.optimize or mindquantum')
