...
def run_gs(mol:MolecularData, ham_op:QubitOperator):
...
def run_es(mol:MolecularData, ham_op:QubitOperator, gs_sim:Simulator):
    # Construct hartreefock wave function circuit
    hartreefock_wfn_circuit = Circuit([X.on(i) for i in range(mol.n_electrons)])
    ...
    ip_grad_ops = es_sim.get_expectation_with_grad(
        Hamiltonian(QubitOperator("")), es_circ, circ_left=Circuit(), simulator_left=gs_sim
    )
...
if __name__ == '__main__':
    mol, ham_op = get_mol()
    gs_sim = run_gs(mol, ham_op)
    run_es(mol, ham_op, gs_sim)
